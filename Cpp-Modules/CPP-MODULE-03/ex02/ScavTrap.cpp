...
/*   Updated: 2024/10/19 17:38:25 by hdemirel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ScavTrap::attack(string const &target)
{
    if (_health == 0)
    {
        std::cout << "Child ScavTrap" << _name << " is already dead." << std::endl;
        return;
    }
    else if (_energy == 0)
    {
        std::cout << "Child ScavTrap" << _name << " has no energy to attack." << std::endl;
        return;
    }
    _energy--;
    std::cout << "Child ScavTrap" << _name << " attacks " << target << ", causing [" << _damage << "] points of damage!" << std::endl;
    std::cout << "Child ScavTrap" << _name << " has [" << _energy << "] energy points left." << std::endl;
}
